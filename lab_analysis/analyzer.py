...
# accuracy
nn_ac_m = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
nn_ac_e = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
cb_ac_m = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
cb_ac_e = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}

# average multiplicity
nn_am_m = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
nn_am_e = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
cb_am_m = np.zeros(SPEC_BOUNDS)
cb_am_e = np.zeros(SPEC_BOUNDS)

# scaled variance
nn_sv_m = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
nn_sv_e = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
cb_sv_m = np.zeros(SPEC_BOUNDS)
cb_sv_e = np.zeros(SPEC_BOUNDS)

# labels
lab_am_m = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
lab_am_e = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
lab_sv_m = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
lab_sv_e = {MODES[0]: np.zeros(SPEC_BOUNDS), MODES[1]: np.zeros(SPEC_BOUNDS)}
...
def eval_cb_ac(x_dat, y_dat, x_bnd, y_bnd):
    ...
    ac_val =  np.sum(np.logical_or(np.logical_and((x_dat <= x_bnd), (y_dat <= y_bnd)), \
            np.logical_and((x_dat > x_bnd), (y_dat > y_bnd)))) / dat_size.size
    return ac_val
...
nse_data = {MODES[0]: np.load(DATA_DIR + 'nrg24.npy').flatten(), \
    MODES[1]: np.load(DATA_DIR + 'spec24.npy').flatten(), \
    EMEAS_STR: np.load(DATA_DIR +  'features_sum.npy').flatten()}
emeas_sorted = np.sort(nse_data[EMEAS_STR])
nrg_sorted = np.sort(nse_data[MODES[0]])
...
for spec_bound in range(SPEC_BOUNDS):
    ...
    perc = nse_data[MODES[1]][nse_data[MODES[1]] <= spec_bound].size / nse_data[MODES[1]].size
    PERC_ARR[spec_bound] = perc
    nse_bounds = {MODES[0]: nrg_sorted[int(perc * nse_data[MODES[0]].size)], \
        MODES[1]: spec_bound, \
    ...
    for mode in MODES:
        ...
        for run in range(RUNS):
            ...
            # calculate accuracy
            ac[run] = np.sum(labels_rdy[train_size:] == predictions) / (N_EVENTS - train_size)

            # calculate average multiplicity and scaled variance
            am[run], sv[run] = n_omega_calc(predictions, mul_rdy[train_size:])
...
modes_better = {MODES[0]: 'fwd energy', MODES[1]: 'num. spec.'}
