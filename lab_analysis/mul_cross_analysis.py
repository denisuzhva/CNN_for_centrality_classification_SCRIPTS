...
sh_ep_nrg_pred = np.load('./test_predictions_nrg_SHIELD_EPOSa_CMA.npy').reshape((ep_size, 2)).argmax(axis=1)
ep_sh_nrg_pred = np.load('./test_predictions_nrg_EPOSa_CMA_SHIELD.npy').reshape((sh_size, 2)).argmax(axis=1)

sh_ep_spec_pred = np.load('./test_predictions_spec_SHIELD_EPOSa_CMA.npy').reshape((ep_size, 2)).argmax(axis=1)
ep_sh_spec_pred = np.load('./test_predictions_spec_EPOSa_CMA_SHIELD.npy').reshape((sh_size, 2)).argmax(axis=1)


## Also load validation predictions

sh_nrg_pred = np.load('./test_predictions_nrg_SHIELD_SHIELD.npy').reshape((sh_size_val, 2)).argmax(axis=1)
ep_nrg_pred = np.load('./test_predictions_nrg_EPOSa_CMA_EPOSa_CMA.npy').reshape((ep_size_val, 2)).argmax(axis=1)

sh_spec_pred = np.load('./test_predictions_spec_SHIELD_SHIELD.npy').reshape((sh_size_val, 2)).argmax(axis=1)
ep_spec_pred = np.load('./test_predictions_spec_EPOSa_CMA_EPOSa_CMA.npy').reshape((ep_size_val, 2)).argmax(axis=1)


## Load cross-labels and normal labels

sh_ep_nrg_lab = np.load('../../DATASETS/NA61_EPOSa_CMA/all/labels_nrg_SHIELD.npy').argmax(axis=1)
sh_ep_nrg_lab = sh_ep_nrg_lab[:ep_size]
ep_sh_nrg_lab = np.load('../../DATASETS/NA61_SHIELD/all/labels_nrg_EPOSa_CMA.npy').argmax(axis=1)

sh_nrg_lab = np.load('../../DATASETS/NA61_SHIELD/all/labels_nrg.npy').argmax(axis=1)
ep_nrg_lab = np.load('../../DATASETS/NA61_EPOSa_CMA/all/labels_nrg.npy').argmax(axis=1)
ep_nrg_lab = ep_nrg_lab[:ep_size]
sh_spec_lab = np.load('../../DATASETS/NA61_SHIELD/all/labels_spec.npy').argmax(axis=1)
ep_spec_lab = np.load('../../DATASETS/NA61_EPOSa_CMA/all/labels_spec.npy').argmax(axis=1)
ep_spec_lab = ep_spec_lab[:ep_size]


## Load multiplicity data

sh_mul = np.load('../../DATASETS/NA61_SHIELD/all/mul.npy').flatten() 
ep_mul = np.load('../../DATASETS/NA61_EPOSa_CMA/all/mul.npy').flatten() 
ep_mul = ep_mul[:ep_size]


## Load sums of measured energy for cut-based analysis

sh_sum = np.load('../../DATASETS/NA61_SHIELD/all/features_sum.npy').flatten() 
ep_sum = np.load('../../DATASETS/NA61_EPOSa_CMA/all/features_sum.npy').flatten() 
...
## N and Omega by the EPOSa_CMA-trained network
