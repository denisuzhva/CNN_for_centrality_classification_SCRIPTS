...
for spec_bound in range(SPEC_BOUNDS):
    ...
    for mode in MODES:
        ...
        ## CNN IS AN EXCEPTIONAL CASE 
        # load labels
        b_dir = RESULTS_DIR + f'{mode}/{spec_bound}/'
        labels_loc = np.load(b_dir + 'labels.npy').argmax(axis=1)
        labels_rdy = copy(labels_loc)
        mul_loc = np.load(b_dir + 'mul.npy') 
        n_events_loc = mul_loc.size
        train_size = int(0.75 * n_events_loc)
        valid_size = n_events_loc - train_size
        valid_size = valid_size // BATCH_SIZE * BATCH_SIZE
        train_size = n_events_loc - valid_size
        ...
        for run in range(RUNS):
            ...
            if MAKE_SHUFFLE:
                perm = np.load(b_dir + f'perm{run}.npy')
                labels_loc = labels_loc[perm]
                mul_loc = mul_loc[perm]
            predictions = np.load(b_dir + f'prediction{run}.npy').reshape((-1, 2)).argmax(axis=1)
            
            # calculate CNN accuracy
            ac[run] = np.sum(labels_loc[train_size:] == predictions) / valid_size

            # calculate CNN am and sv
            am[run], sv[run] = n_omega_calc(predictions, mul_loc[train_size:])
        ...
        ## HERE COME TRADITIONAL LABELS AND MULTIPLICITIES
